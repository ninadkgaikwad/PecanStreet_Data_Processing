function [PecanStreet_Data_Output] = PecanStreet_Data_Extractor(HEMSWeatherData_Input,PecanStreet_Data_FolderPath,N_House_Vector,Type,Data_MatFile_Name)
...
for ii=1:length(FileNames_Cell) % For each file in the Folder
   ...
   if (length(startIndex)==0) % If actual File
       ...
       AllFiles_Data_Cell(1,ii)= {csvread(FullPathName)};            
...
for ii=1:length(AllFiles_Data_Cell) % For each file AllFiles_Data_Cell
    ...
    DateTimeMatrixAggregate_ForSlicer=horzcat(Single_DataFile_Matrix(:,1:4),zeros(Row,1));
    ...
    if (isstring(OriginalSeries)) % Desired Dates not present in the current file
        ...
        AllFiles_CorrectDates_Data_Matrix(:,:,DateTime_NoError_Counter)=[DateTime_Matrix,Data];
...
if (isnan(AllFiles_CorrectDates_Data_Matrix))
    ...
    AllFiles_CorrectDates_Priority_Data_Matrix=[AllFiles_CorrectDates_Data_Matrix(:,1:4,:)... % Date-Time
                                                ...
                                                AllFiles_CorrectDates_Data_Matrix(:,61+4:62+4,:) AllFiles_CorrectDates_Data_Matrix(:,25+4,:) AllFiles_CorrectDates_Data_Matrix(:,39+4,:) AllFiles_CorrectDates_Data_Matrix(:,374,:) AllFiles_CorrectDates_Data_Matrix(:,40+4,:) AllFiles_CorrectDates_Data_Matrix(:,38+4,:) AllFiles_CorrectDates_Data_Matrix(:,60+4,:) AllFiles_CorrectDates_Data_Matrix(:,71+4,:) AllFiles_CorrectDates_Data_Matrix(:,49+4,:) AllFiles_CorrectDates_Data_Matrix(:,53+4:54+4,:) ... Level 1 Priority - Fridge, Freezer, Kitchen
    ...
    % Only PV_Bat Indices
    House_Only_PV_Bat_File_Indices=intersection(House_OnlyPV_File_Indices,House_OnlyBat_File_Indices);

    % Only PV_EV Indices
    House_Only_PV_EV_File_Indices=intersection(House_OnlyPV_File_Indices,House_OnlyEV_File_Indices);

    % Only Bat_EV Indices
    House_Only_Bat_EV_File_Indices=intersection(House_OnlyBat_File_Indices,House_OnlyEV_File_Indices);
